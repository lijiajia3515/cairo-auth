package com.hfhk.auth.service.auth.service.impl;
...
import com.hfhk.auth.service.auth.constants.AuthConstant;
import com.hfhk.auth.service.auth.converter.ResourceConverter;
...
import com.hfhk.auth.service.auth.service.ResourceService;
...
public class ResourceServiceImpl implements ResourceService {
	...
	public List<ResourceTreeNode> treeFindByUid(String client, String uid) {
		Query userQuery = Query.query(Criteria.where("uid").is(uid));
		userQuery.fields().include("clientRoles").include("clientResources");
				...
				return Optional.ofNullable(user.getClientRoles())
					...
					.flatMap(x -> {
						if (!x.contains(RoleConstant.ADMIN)) {
							Query roleQuery = Query.query(Criteria.where("code").in(x));
							...
							return Optional.of(Stream.concat(userResources, roleResources)
								...
								.filter(y -> !y.isEmpty())
								.map(y -> mongoTemplate.find(Query.query(Criteria.where("_id").in(y)), ResourceMongo.class));

						} else {
							return Optional.of(mongoTemplate.find(Query.query(Criteria.where("client").is(client)), ResourceMongo.class));
						}
					})
					.stream()
					.filter(x -> !x.isEmpty())
	...
	public ResourceTreeNode save(String client, ResourceSaveRequest request) {
		ResourceMongo data = ResourceMongo.builder()
			.client(client)
			.parentId(Optional.ofNullable(request.getParentId()).orElse(AuthConstant.RESOURCE_TREE_ROOT))
	...
	public void move(String client, ResourceMoveRequest request) {
		Query query = Query.query(
			Criteria
				...
				.and("_id").in(request.getIds())
		);

		Update update = Update.update("parentId", request.getMovedParentId());
	...
	public ResourceTreeNode findById(String client, String id) {
		Comparator<ResourceTreeNode> comparator = Comparator.comparing(ResourceTreeNode::getSort).thenComparing(ResourceTreeNode::getId);
